...
public class GenerateNature : MonoBehaviour
{
    ...
    public static void GenerateMobWithHeight(bool useFalloff, int seed, float noiseScale, int octaves, float persistance, float lacunarity, Vector2 center, Vector2 offset, Noise.NormalizeMode normalizeMode, float[,] falloffMap, TerrainType[] regions, AnimationCurve meshHeightCurve, float meshHeightMultiplier, GameObject parentPrefab)
    {
        float[,] noiseMap;
        noiseMap = Noise.GenerateNoiseMap(mapChunkSize + 2, mapChunkSize + 2, seed, noiseScale, octaves, persistance, lacunarity, center + offset, normalizeMode);
        int nbPrefab;
