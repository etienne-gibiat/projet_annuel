...
namespace Pinwheel.CsharpWizard
{
    public partial class ClassWizard : EditorWindow
    {
        ...
        {
            MonoBehaviour, ScriptableObject, Class, Struct, Interface, Enum, Inspector, InspectorDrawer, Window
        }
        ...
        private static string NamespaceName
        {
            get
            {
                if (string.IsNullOrEmpty(namespaceName))
                    namespaceName = CSharpWizardHelper.GetDefaultNamespace(directory);
        ...
        public static string Generate()
        {
            ...
            string[] text = scriptType switch
            {
                ScriptType.MonoBehaviour => GenerateMonoBehaviour(),
                ScriptType.Class => GenerateClass(),
                ScriptType.Struct => GenerateStruct(),
                ScriptType.Interface => GenerateInterface(),
                ScriptType.Enum => GenerateEnum(),
                ScriptType.Inspector => GenerateInspector(),
                ScriptType.InspectorDrawer => GenerateInspectorDrawer(),
                ScriptType.Window => GenerateWindow(),
                ScriptType.ScriptableObject => GenerateScriptableObject(),
                _ => null
            };
        ...
        public static string[] GenerateMonoBehaviour()
        {
            CompleteBaseClassAndInterfaceList();
            var d = new TypeDeclaration.Builder()
                ...
                .SetName(ClassName)
                .SetBaseClass(typeof(PGMonoBehaviour))
                .SetInterfaces(implementedInterfaces.ToArray())
                .Build();
            
            var un = CSharpWizardHelper.GetUsingNode();
            ...
            {
                foreach (var type in d.implementedInterface)
                {
                    un.Using(type.Namespace);
                }
            }
            
            TypeAlias.SetUsingNode(un);
            
            List<string> code = CSharpWizardHelper.GetCustomMonoBehaviourCode();
            
            
            var tn = new TypeNode()
                .Declare(d)
                .Implement(code);
            var nn = new NamespaceNode()
                .SetName(NamespaceName)
                .SetTypes(tn);
            
            var sn = new ScriptNode()
            ...
            var script = formatter.Format(sn);
        ...
        public static string[] GenerateClass()
        {
            ...
            UsingNode un = CSharpWizardHelper.GetUsingNode();
            ...
            if (actualBaseClass != null)
            {
                ...
                {
                    code = CSharpWizardHelper.GetCustomMonoBehaviourCode();
                }
            }
        ...
        public static string[] GenerateInspector()
        {
            ...
            //Import regularly used namespaces, just call a Using(...) if you need more
            UsingNode un = CSharpWizardHelper.GetUsingEditorNode();
        ...
        private static void SaveSettings()
        {
            ...
            EditorPrefs.SetString(EditorCommon.GetProjectRelatedEditorPrefsKey(CSHARP_WIZARD, "NamespaceName"), string.Empty);
