...
namespace I2.Loc
{
	public partial class LocalizationEditor
	{
		...
		void BuildScriptWithSelectedTerms()
		{
            ...
            sbTrans.AppendLine();
            sbTrans.AppendLine( "namespace PGSauce.Localization" );
            sbTrans.AppendLine( "{" );
		...
		static string GetPathToGeneratedScriptLocalization()
		{
			...
			return $"Assets/{PGSettings.AutomaticallyGeneratedPath}/ScriptLocalization.cs";
        }
		...
		void BuildScriptCategory( StringBuilder sbTrans, StringBuilder sbTerms, string Category, List<string> AdjustedTerms, List<string> Terms )
		{
			if (Category==LanguageSourceData.EmptyCategory)
			{
                ...
                {
                    sbTrans.AppendLine( "		public static string " + AdjustedTerms[i] + " \t\t{ get{ return LocalizationManager.GetTranslation (\"" + Terms[i] + "\", applyParameters : true); } }");
			...
			for (int i=0; i<Terms.Count; ++i)
			{
				sbTrans.AppendLine("			public static string "+AdjustedTerms[i]+ " \t\t{ get{ return LocalizationManager.GetTranslation (\"" + Category+"/"+Terms[i]+"\", applyParameters : true); } }");
