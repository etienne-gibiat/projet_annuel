...
namespace SuperPivot
{
    ...
    public class TransformCustomInspector : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            Vector3 pos = ((Transform)this.target).position;
            Vector3 eulers = ((Transform)this.target).eulerAngles;
            Vector3 scale = ((Transform)this.target).lossyScale;

            EditorGUILayout.LabelField("World Space", EditorStyles.boldLabel);
            GUI.enabled = false;
            EditorGUILayout.Vector3Field("Position", pos);
            EditorGUILayout.Vector3Field("Rotation", eulers);
