	...
	public class SWShaderProcessRoot:SWShaderProcessBase{
		...
		protected void ProcessSpriteNormal(SWNodeBase _node)
		{
			...
			if (totalimportance > 0) {
				...
				StringAddLine (string.Format ("\t\t\t\tresultNormal /= {0};", totalimportance.ToStringEX()));
				StringAddLine (string.Format ("\t\t\t\to.Normal = lerp(o.Normal,resultNormal,{0});", Mathf.Clamp01 (totalimportance).ToStringEX()));
			}
		}
		#endregion
