...
namespace ShaderWeaver
{
	...
	public class RMBItem
	{
		public string name;
		public System.Action act;
		public System.Func<bool> func_valid;

		public RMBItem(string _name, System.Action _act, System.Func<bool> _func_valid)
		{
			name = _name;
			act= _act;
			func_valid = _func_valid;
		}
	}
	...
	public partial class SWWindowLayoutV : SWWindowBase {
		...
		{
			RMBMenu_Show (RMBMenu_PosOut + new Vector2(0,al_startY));
		}
...
//		}
		...
		/// <summary>
		/// For Mask Node's wand and dropper
		/// Operate on first visible node in LayerMask
		/// </summary>
		public SWNodeBase LayerMask_WandDropperNode(SWNodeBase node)
		{
			foreach (var item in SWWindowMain.Instance.nodes) {
				if (item.data.name == node.data.layerPop_str)
					return item;
			}
			return null;
		}
		...
		protected void ShowLayerPopup(SWNodeBase node)
		{
			int index = 0;
			LayerMaskUpdate (node);
			var layerMask = node.data.layerMask;
			...
			for (int i = 0; i < strs.Length; i++) {
				strs [i] = ns [layerMask.strs [i]].data.name;
				if (strs [i] == node.data.layerPop_str)
					index = i;
			}
				
			Rect rect = new Rect (al_leftWidth + 10, al_topHeight + 10, 73, 10);
			index = EditorGUI.Popup (rect,index, strs);
			node.data.layerPop_str = strs [index];
		}
		#region RMBMenu
		[SerializeField]
		protected List<RMBItem> RMBMenu_List;
		[SerializeField]
		protected Vector2 RMBMenu_Pos;
		[SerializeField]
		protected Vector2 RMBMenu_PosOut;

		[SerializeField]
		protected bool RMBMenu_isShowing = false;
		/// <summary>
		/// Can be press after fisrt rendering frame
		/// </summary>
		[SerializeField]
		protected bool RMBMenu_hasShow = false;

		/// <summary>
		///  RMBMenu: Start->Init->Show->Clean
		/// </summary>
		protected virtual void RMBMenu_Start()
		{
			RMBMenu_isShowing = true;
			repaintDirty = true;
			RMBMenu_PosOut = mousePosOut;
			RMBMenu_Pos = mousePos;
		}
		protected virtual void RMBMenu_Init()
		{
		...
		protected virtual void RMBMenu_Clean()
		{
			RMBMenu_hasShow = false;
		}
		#endregion
	}
}