					...
					if (directChildID == item.data.id) {
						if(item.data.type == SWNodeType.alpha)
							StringAddLine (string.Format ("\t\t\t\t{0} = clamp({1}*{2},{3},{4});",
								alphaParam,item.param,item.opFactor,item.data.effectData.pop_min.ToStringEX(), item.data.effectData.pop_max.ToStringEX()));
						else
			...
			for (int i = 0; i < node.data.gradients.Count; i++) {
				...
				if (frames.Count == 0) {
					...
					for (int j = 0; j < MaxCount; j++) {
						if (j < frames.Count)
							strList += ("" + node.data.gradients [i].frames [j].time.ToStringEX());
						else
							strList += ("-1");
						if (j != MaxCount - 1)
							strList += (",");
					...
					for (int j = 0; j < MaxCount; j++) {
						if (j < frames.Count)
							strList += ("" + node.data.gradients [i].frames [j].value.ToStringEX());
						else
							strList += ("-1");
						if (j != MaxCount - 1)
							strList += (",");
