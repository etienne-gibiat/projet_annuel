...
namespace PGSauce.Core.DataStructures
{
    /// <summary>
	...
	public static class Combinatorial
	{
		...
		public static IEnumerable<IEnumerable<IEnumerable<T>>> Partitions<T>(this IEnumerable<T> list)
		{
			...
			var elementCount = listArray.Length;
		...
		private static IEnumerable<T> PermutationsImpl<T>(int n, Func<int[], T> select)
		{
			...
			while (true)
			{
				...
				PGDebug.SetCondition(index >= 0).Message("index must be greater than or equal to 0").Assert();
				PGDebug.SetCondition(index + 1 < permutation.Length).Message("index + 1 must be smaller than permutation length, but index == " + index).Assert();
